...
import { EditMapDetailsModal } from "./components";
import {
  useGetPlaceLocation,
  useGetMapInfo,
  usePutEditMap,
...
const EditMapPage = () => {
  ...
  const { data: mapInfo, refetch: mapInfoRefetch } = useGetMapInfo({
    userId,
    mapId,
  });

  const [search, setSearch] = useState<string>("");
  const [showEditMapDetailsModal, setShowEditMapDetailsModal] = useState(false);
  ...
  const { mutate: editMapMutate } = usePutEditMap();
  ...
  const handleClickEditModal = () => {
    setShowEditMapDetailsModal((prev) => !prev);
  };
  ...
  return (
    <div className="w-screen h-dvh flex flex-col">
      {showEditMapDetailsModal && (
        <EditMapDetailsModal
          onClose={setShowEditMapDetailsModal}
          mapData={mapData}
            ...
            {openSearchResultMenu ? (
              ...
              <>
                <div className="flex flex-col gap-4">
                  ...
                  <button
                    className="w-full h-[34px] rounded-2xl bg-[#FFE852] font-semibold hover:bg-yellow-300"
                    type="button"
                ...
                <div className="flex shrink-0 h-fit w-full pb-7 sm:pb-10">
                  <button
                    className="w-full h-12 rounded-full font-semibold leading-5 bg-[#FFE852] disabled:bg-[#EDEDED] hover:bg-yellow-300"
                    disabled={
                      addedPlaces.length === 0 ||
                      mapData.title === "" ||
                      mapData.description === "" ||
